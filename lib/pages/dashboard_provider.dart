...
class DashboardProvider extends ChangeNotifier {
  ...
  getExploreData(BuildContext context) async {
    ...
    await userCol
        .where(FirestoreContants.id_user, whereNotIn: matchList)
        .get()
        .then((value) async {
      exploreData = [];
  ...
  uploadImageToFirebase(File? file) async {
    ...
    try {
      Reference reference = FirebaseStorage.instance
          .ref()
          .child('${currentUserModel?.email}/photos/$fileName');
  ...
  onUpdateProfile(UserModel? model, context,
      {String msg = 'Dog details updated'}) {
  ...
  sendMatchRequest(UserModel? model, context) async {
    if (model?.id != null) {
      ...
      model?.matchRequests
          .add(FirebaseFirestore.instance.doc('users/${currentUserModel?.id}'));
  ...
  acceptMatchRequest(UserModel? model, context) async {
    if (model!.id != null) {
      currentUserModel?.matchRequests
  ...
  sendBreedingRequest(UserModel? model, context) async {
    if (model!.id != null) {
      DocumentReference? doc =
          FirebaseFirestore.instance.doc("users/${model.id}");
      ...
      model.breedingRequests
          .add(FirebaseFirestore.instance.doc('users/${currentUserModel?.id}'));
      await doc.update(model.toJson());
      if (currentUserModel?.gender == 'Female') {
        AwesomeDialog(
          context: context,
          dialogType: DialogType.success,
          animType: AnimType.scale,
